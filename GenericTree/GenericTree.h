...
static void treeFactory(GenericTree<int>& tree) {
  ...
  //create the root node (4)
  GenericTree<int>::TreeNode* root = tree.createRoot(4);

  //add two children to root (8 and 15)
  GenericTree<int>::TreeNode* node8 = root->addChild(8);
  GenericTree<int>::TreeNode* node15 = root->addChild(15);

  //add two children to node8 (16 and 23)
  GenericTree<int>::TreeNode* node16 = node8->addChild(16);
  GenericTree<int>::TreeNode* node23 = node8->addChild(23);

  //add one child to node16 (42)
  node16->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::queue<TreeNode*> nodesToExplore; //queue for level order traversal
  
  nodesToExplore.push(rootNodePtr); //push root node

  //level order traversal
  while(!nodesToExplore.empty()){
    
    TreeNode* currentNode = nodesToExplore.front(); //node at front of the queue is 'currentNode'
    
    nodesToExplore.pop(); //remove node from queue to ensure the next node is processed next
    
    //ensure current node is not null
    if(currentNode != nullptr){
      results.push_back(currentNode->data);

      //for loop to iterate over current node's children
      for(unsigned int i = 0; i < currentNode->childrenPtrs.size(); ++i){
        
        TreeNode* childPtr = currentNode->childrenPtrs[i]; //ptr to current child
          
        //only enqueue not null childPtrs
        if (childPtr != nullptr){

          nodesToExplore.push(childPtr); //add child to queue for further processing
