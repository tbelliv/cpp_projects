...
void downHeap(Node *n){
    ...
    //pointer to track the smallest child
    Node *smallest = n;

    //compare n with its left child if it exists
    if(n->left != nullptr && n->left->value < n->value){
        smallest = n->left;
    }
    
    //compare n with the right child if it exists
    if(n->right != nullptr && n->right->value < smallest->value){
        smallest = n->right;
    }
    
    //if the smallest value is not n, swap
    if(smallest != n){
        std::swap(n->value, smallest->value); // Swap the values
        
        //recursive call to downHeap on the child that was swapped
        downHeap(smallest);
    }
}
