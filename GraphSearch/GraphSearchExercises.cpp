...
int GridGraph::countEdges() const {
    ...
    //iterate over each point and count the size of its NeighborSet
    for (const auto& entry : adjacencyMap) {
        numEdges += entry.second.size();
    }

    //each edge is currently counted twice (once for each incident vertex)
    //divide by 2 to solve for this
    numEdges /= 2;
...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  //remove p1 from each of its neighbors' NeighborSet
    for (const auto& neighbor : originalNeighbors) {
        adjacencyMap[neighbor].erase(p1);
    }

    //remove p1 from the adjacencyMap
...
std::list<IntPair> graphBFS(const IntPair& start, const IntPair& goal, const GridGraph& graph) {
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
    ...
    for (auto neighbor : neighbors) {
      ...
      bool neighborWasAlreadyVisited = visitedSet.count(neighbor); //edit
      // ==================================================================
...
std::list<PuzzleState> puzzleBFS(const PuzzleState& start, const PuzzleState& goal) {
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
    ...
    auto neighbors = curState.getAdjacentStates(); // Change this! This line is totally wrong. //edit

    // Hint: Look at PuzzleState.h //maybe correct
    // =====================================================================
    ...
    for (auto neighbor : neighbors) {
      ...
      // TODO: Your code here!
      // Check whether the neighbor has already been visited.
      bool neighborWasAlreadyVisited = visitedSet.count(neighbor) > 0; //edit
      // ==================================================================
