...
void LinkedList<T>::insertOrdered(const T& data){
    ...
    //make new node the head_ and tail_ of 'this' if there are no other nodes
    ...
    //make new node take the place of the current head if it is smaller in value than head
    
    if(data < this->head_->data){
    
      newNode->next = this->head_;
      
      this->head_->prev = newNode; //make head prev point to newNode
      
      this->head_ = newNode; //update the head to our newNode
    ...
    //make new node take the place of the current tail if it is larger in value than tail
    
    if(data > this->tail_->data){
    ...
    //insert new node into the middle of the list
    
    Node* current = this->head_;
    
    while(current != nullptr){
        
      //locates the first node that has a greater value than the new node's data
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  /*### ✗ - [0/1] - Testing merge: Left list empty; right list non-empty

- **Points**: 0 / 1

An unknown error occurred processing this test. Please contact course staff.



### ✗ - [0/1] - Testing merge: Left list non-empty; right list empty
...
### ✗ - [0/1] - Testing merge: Left and right lists non-empty; same size
...
### ✗ - [0/1] - Testing merge: Left and right lists non-empty; left list is longer
...
### ✗ - [0/1] - Testing merge: Left and right lists non-empty; right list is longer

- **Points**: 0 / 1*/

  //pointers to traverse the left and right head nodes at current baseline positions
  
  typename LinkedList<T>::Node* leftcurrent = left.head_;
  
  typename LinkedList<T>::Node* rightcurrent = right.head_;
  ...
  while(leftcurrent != nullptr && rightcurrent != nullptr){
    ...
    if(leftcurrent->data <= rightcurrent->data){
      ...
      //add the smaller or equal data from left to merged list
        
      merged.pushBack(leftcurrent->data);
        
      leftcurrent = leftcurrent->next;  //move to the next node in left
        
    } 
        
    else{
      
      //add the smaller data from right to merged list
      
      merged.pushBack(rightcurrent->data);
      
      rightcurrent = rightcurrent->next; //move to the next node in right
    
    }
  }

  //if there are remaining nodes in the left list, add them to the merged list
  ...
  //if there are remaining nodes in the right list, add them to the merged list
