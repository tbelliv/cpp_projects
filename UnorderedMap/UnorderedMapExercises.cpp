...
StringIntMap makeWordCounts(const StringVec& words) {
  // Declare an unordered_map of strings to integers (word to count)
  StringIntMap wordcount_map;

  // Iterate over the vector of words and count occurrences
...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  ...
  auto it = wordcount_map.find(key); // auto for iterator type deduction
  return (it != wordcount_map.end()) ? it->second : fallbackVal; // return mapped value or fallbackVal
}
